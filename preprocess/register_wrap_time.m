...
% % INPUT CONTROL 
% if nargin  <3
% selected_trials = 1:size(rawmov,3);
% end
% 
...
for idx = selected_trials
    ...
    [tform]= imregtform(rawmov(:,:,idx),ref_frame, 'rigid', optimizer, metric); 
    Rfixed = imref2d(size(ref_frame));% coordinateds
    
    registmov(:,:,idx) = imwarp(rawmov(:,:,idx),tform,'OutputView',Rfixed, 'SmoothEdges', false);
